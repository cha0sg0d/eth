...
async function noKeyWhitelistRegister(args: { path: string }, hre: HardhatRuntimeEnvironment) {
  ...
  console.log('whitelist balance before adding:', parseInt(hre.ethers.utils.formatEther(prevBalance)));
  ...
  if( (parseFloat(drip) * validAddresses.length) > parseInt(hre.ethers.utils.formatEther(prevBalance))) {
  ...
  for(let i = 0; i < validAddresses.length; i+=slice) {
    ...
    if( parseFloat(drip) * addressChunk.length > parseInt(hre.ethers.utils.formatEther(prevBalance))) {
      console.log("not enough eth in contract. Add more before whitelisting");
      break;
    }
    console.log('whitelist balance before adding:', parseInt(hre.ethers.utils.formatEther(prevBalance)));
